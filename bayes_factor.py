...
class Bayes_factor(object):
    ...
    def __call__(self,params):
        ...
        return self.flikelihood(fparams)/self.alikelihood(aparams) #Baye's factor
...
bayes = np.empty((np.shape(flattened)[0],1))

rangelength = 15.0

print ('If Bayes factor is >>1 then flattened model is preferred over asymmetry model.')
...
for j in range(np.shape(flattened)[0]):
    ...
    bayes_inst = Bayes_factor(Flike,Alike)
    ...
    bayes[j] = bayes_inst(params)
    print ('for mode:',j+1,'   Bayes factor = ', bayes[j,0])
