...
class Model(object):
    ...
    def __call__(self, params):
        ...
        X1 = self.X_value([frequency1,width1,splitting1])
        X2 = self.X_value([frequency2,width2,splitting2])
        
        W1 = -epsilon1**2 + X1**2
        W2 = -epsilon2**2 + X2**2
        #W1 = np.asarray([np.abs(x) for x in W1_temp])
        #W2 = np.asarray([np.abs(x) for x in W2_temp])
        ...
        #l=1 step
        m += self.height([amplitude1,width1])/(2.0*epsilon1) * np.arctan((2.0*epsilon1/(1.+ W1)))
        #l=2 step
        m += self.height([amplitude2,width2])/(2.0*epsilon2) * np.arctan((2.0*epsilon2/(1.+ W2)))
